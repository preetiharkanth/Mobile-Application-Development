...
public class NotesDao {
    ...
    public boolean update(Notes notes) {
        ...
        values.put(NotesTable.COLUMN_DATE, notes.getDate());
        values.put(NotesTable.COLUMN_NOTES, notes.getNote());
        return db.update(NotesTable.TABLE_NAME, values, NotesTable.COLUMN_CITYID + "=?", new String[]{notes.getCityKey() + ""}) > 0;
    }

    public boolean delete(Notes notes) {
        return db.delete(NotesTable.TABLE_NAME, NotesTable.COLUMN_CITYID + "=?", new String[]{notes.getCityKey() + ""}) > 0;
    }
